...
const StallSlot = ({
  ...
  const getBaseColors = () => {
    if (type === "anchor") return "bg-gradient-to-br from-[#FFE8CC] to-[#FDCB95] border-[#D19B5E] text-[#69421A] shadow-[inset_0_1px_2px_rgba(255,255,255,0.7)]"; // Elegant gradient orange
    if (type === "other") return "bg-gradient-to-br from-white to-slate-50 border-slate-300 text-slate-500 shadow-[inset_0_1px_2px_rgba(255,255,255,0.8)]";
  ...
  const getStatusColors = () => {
    if (status === "occupied") return "bg-red-200 border-red-300 text-red-700 cursor-not-allowed opacity-70";
    if (status === "reserved") return "bg-yellow-100 border-yellow-200 text-yellow-700 cursor-not-allowed opacity-70";
    if (isSelected) return "ring-2 ring-primary ring-offset-[1px] scale-[1.03] z-20 shadow-lg shadow-primary/30";
  ...
  return (
    ...
    >
      {status === "occupied" ? (
        <span className="text-[9px] font-black text-red-500/80 rotate-[-12deg] tracking-tighter uppercase border border-red-500/30 px-1 rounded-sm shadow-[0_0_10px_rgba(239,68,68,0.1)]">SOLD</span>
      ) : (
        label
      )}
      {isSelected && (
...
const StallBlock = ({
  ...
  return (
    <div className="relative w-full">
      ...
      <div className={`flex w-full h-[120px] gap-[6px] p-1.5 bg-white/50 border border-slate-200/60 rounded-sm shadow-sm ${side === "right" ? "flex-row-reverse" : "flex-row"}`}>
        ...
        <div className="grid grid-cols-4 grid-rows-2 gap-[6px] w-[66%] h-full">
          {topRow.map((label) => (
            <StallSlot
              ...
              label={label}
              type="standard"
              status={occupiedStalls.includes(`${anchorId}-${label}`) ? "occupied" : "available"}
        ...
        <div className="flex-1 h-full">
          <StallSlot
            ...
            label={anchorId}
            type="anchor"
            status={occupiedStalls.includes(anchorId) ? "occupied" : "available"}
...
export default function BookYourSlotPage() {
  ...
  useEffect(() => {
    ...
    const fetchOccupiedStalls = async () => {
      try {
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/exhibitor-registrations/occupied_stalls/`);
        const data = await res.json();
      ...
      <div className="pt-24 pb-12 px-2 md:px-4 font-sans text-slate-900">
        <div className="max-w-[1400px] mx-auto">
          ...
          <div className="text-center mb-10">
            <h1 className="text-4xl md:text-5xl font-serif mb-4 text-emerald-950">Exhibition Floor Plan</h1>
            <p className="text-slate-600 max-w-2xl mx-auto">
              Select your preferred exhibition booth from our interactive floor plan.
            </p>
          </div>
          ...
          <div className="grid lg:grid-cols-12 gap-8 items-start">
            ...
            <div className="lg:col-span-8 bg-white p-2 md:p-8 rounded-2xl shadow-xl border border-border/50">
              <div className="mb-8 flex flex-wrap items-center justify-between gap-4 border-b pb-6 mx-2">
                <div className="flex gap-6">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs font-bold text-slate-500 uppercase tracking-tight">Standard (3x4)</span>
                  </div>
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs font-bold text-slate-500 uppercase tracking-tight">Anchor (6x8)</span>
                  </div>
                </div>
              </div>
              ...
              <div className="w-full overflow-x-auto pb-6 custom-scrollbar flex justify-center">
                <div style={{ width: '820px', minWidth: '820px', height: '1700px', position: 'relative' }}>
                  <div
                  ...
                  >
                    ...
                    <div className="pt-6 pb-0 px-0 w-full h-full flex flex-col justify-between relative">
                      ...
                      <div className="flex justify-between w-full">
                        <div className="w-[44%]"><StallBlock side="left" anchorId="A6" pavilionLabel="P6" selectedStall={selectedStall} onSelect={handleStallSelect} occupiedStalls={occupiedStalls} /></div>
                        <div className="w-[44%]"><StallBlock side="right" anchorId="A7" pavilionLabel="P7" selectedStall={selectedStall} onSelect={handleStallSelect} occupiedStalls={occupiedStalls} /></div>
                      </div>

                      <div className="flex justify-between w-full mt-4">
                        <div className="w-[44%]"><StallBlock side="left" anchorId="A5" pavilionLabel="P5" selectedStall={selectedStall} onSelect={handleStallSelect} occupiedStalls={occupiedStalls} /></div>
                        <div className="w-[44%]"><StallBlock side="right" anchorId="A8" pavilionLabel="P8" selectedStall={selectedStall} onSelect={handleStallSelect} occupiedStalls={occupiedStalls} /></div>
                      </div>
                      ...
                      <div className="flex justify-between items-center w-full h-8 relative my-2">
                        <div className="absolute left-[-22px] w-8 h-8 bg-white border border-slate-400 flex items-center justify-center shadow-sm z-20">
                          <span className="text-[10px] text-slate-700 tracking-widest uppercase">G3</span>
                        </div>
                        <div className="absolute right-[-22px] w-8 h-8 bg-white border border-slate-400 flex items-center justify-center shadow-sm z-20">
                          <span className="text-[10px] text-slate-700 tracking-widest uppercase">G6</span>
                        </div>
                      </div>

                      <div className="flex justify-between w-full">
                        <div className="w-[44%]"><StallBlock side="left" anchorId="A4" pavilionLabel="P4" selectedStall={selectedStall} onSelect={handleStallSelect} occupiedStalls={occupiedStalls} /></div>
                        <div className="w-[44%]"><StallBlock side="right" anchorId="A9" pavilionLabel="P9" selectedStall={selectedStall} onSelect={handleStallSelect} occupiedStalls={occupiedStalls} /></div>
                      </div>
                      ...
                      <div className="flex justify-between w-full h-[360px] relative my-6">
                        ...
                        <div className="flex flex-col items-start h-full w-[38%] pt-6 pl-6">
                          ...
                          <div className="h-48 w-24 bg-gradient-to-br from-[#FEFCE8] to-[#F7F0B0] border border-[#CAB85D] flex items-center justify-center shadow-[inset_0_1px_2px_rgba(255,255,255,0.7)] transform transition hover:-translate-y-0.5 ml-20 -mt-px rounded-sm relative z-0">
                            <span className="-rotate-90 text-[11px] font-medium text-[#7D7122] tracking-widest uppercase whitespace-nowrap">Cafeteria</span>
                          </div>
                      ...
                      {/* LOWER SECTION */}
                      <div className="flex justify-between w-full mt-4">
                        <div className="w-[44%]"><StallBlock side="left" anchorId="A3" pavilionLabel="P3" selectedStall={selectedStall} onSelect={handleStallSelect} occupiedStalls={occupiedStalls} /></div>
                        <div className="w-[44%]"><StallBlock side="right" anchorId="A10" pavilionLabel="P10" selectedStall={selectedStall} onSelect={handleStallSelect} occupiedStalls={occupiedStalls} /></div>
                      </div>

                      <div className="flex justify-between w-full mt-8">
                        <div className="w-[44%]"><StallBlock side="left" anchorId="A2" pavilionLabel="P2" selectedStall={selectedStall} onSelect={handleStallSelect} occupiedStalls={occupiedStalls} /></div>
                        <div className="w-[44%]"><StallBlock side="right" anchorId="A11" pavilionLabel="P11" selectedStall={selectedStall} onSelect={handleStallSelect} occupiedStalls={occupiedStalls} /></div>
                      </div>

                      <div className="flex justify-between w-full mt-8">
                        <div className="w-[44%]"><StallBlock side="left" anchorId="A1" pavilionLabel="P1" selectedStall={selectedStall} onSelect={handleStallSelect} occupiedStalls={occupiedStalls} /></div>
                        <div className="w-[44%]"><StallBlock side="right" anchorId="A12" pavilionLabel="P12" selectedStall={selectedStall} onSelect={handleStallSelect} occupiedStalls={occupiedStalls} /></div>
                      </div>
                      ...
                      <div className="relative w-full mt-10 mb-0 h-20 px-6">
                        ...
                        <div className="absolute top-1 left-[-22px] w-8 h-12 bg-white border border-slate-400 flex items-center justify-center shadow-sm z-20">
                          <span className="text-[10px] text-slate-700 tracking-widest uppercase">G4</span>
                        </div>
                        <div className="absolute top-1 right-[-22px] w-8 h-12 bg-white border border-slate-400 flex items-center justify-center shadow-sm z-20">
                          <span className="text-[10px] text-slate-700 tracking-widest uppercase">G7</span>
                        </div>
                        ...
                        <div className="absolute bottom-[-22px] left-[54px] w-8 h-6 bg-white border border-slate-400 flex items-center justify-center shadow-sm z-20">
                          <span className="text-[10px] text-slate-700 tracking-widest">G10</span>
                        </div>
                        <div className="absolute bottom-[-22px] left-[242px] w-8 h-6 bg-white border border-slate-400 flex items-center justify-center shadow-sm z-20">
                          <span className="text-[10px] text-slate-700 tracking-widest">G9</span>
                        </div>

                        <div className="flex w-full h-full items-end justify-between px-2">
                          <div className="flex items-end h-full gap-4">
                            <div className="w-10 h-6"></div> {/* Spacer for G10 gap */}
                          ...
                          <div className="flex items-end h-full gap-5 pb-0">
                            <div className="w-6 h-6"></div> {/* Spacer for G9 gap */}
              ...
              <div className="sticky top-28 space-y-6">
                  ...
                  {selectedStall ? (
                    ...
                    >
                      <div className="flex justify-between items-start mb-8">
                        <div>
                          <span className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest mb-3 inline-block ${!selectedStall.includes('-') ? 'bg-orange-100 text-orange-700' : 'bg-slate-100 text-slate-600'}`}>
                            {!selectedStall.includes('-') ? 'PREMIUM ANCHOR' : 'STANDARD BOOTH'}
                          </span>
                          <h2 className="text-4xl font-serif text-slate-900 tracking-tight leading-none">{selectedStall}</h2>
                          <div className="mt-3 inline-block bg-primary/5 py-1.5 px-3 rounded-lg border border-primary/10">
                            <span className="text-[10px] font-black text-primary uppercase tracking-widest block opacity-50 mb-0.5">Pavilion Category</span>
                            <p className="text-xs font-bold text-slate-700">
                              {getPavilionInfo(selectedStall)}
                            </p>
                          </div>
                        </div>
                        <div className={`${occupiedStalls.includes(selectedStall) ? 'bg-red-500 shadow-red-200' : 'bg-emerald-500 shadow-emerald-200'} text-white px-4 py-2 rounded-full text-[11px] font-black shadow-lg uppercase tracking-wider`}>
                          {occupiedStalls.includes(selectedStall) ? 'SOLD OUT' : 'AVAILABLE'}
                        </div>
                      </div>
                      ...
                      <div className="space-y-4 mb-10">
                        <div className="bg-slate-50 rounded-2xl p-5 border border-slate-100 flex items-center gap-4">
                          ...
                          <div>
                            ...
                            <p className="text-base font-bold text-slate-700">Main Exhibition Hall</p>
                          </div>
                        </div>
                        <div className="bg-slate-50 rounded-2xl p-5 border border-slate-100 flex items-center gap-4">
                          <div className="w-12 h-12 bg-white rounded-xl shadow-sm flex items-center justify-center text-primary">
                          ...
                          <div>
                            <p className="text-[10px] uppercase font-black text-slate-400 tracking-widest">Specifications</p>
                            <p className="text-base font-bold text-slate-700">{!selectedStall.includes('-') ? '6m x 8m (48 sq.m)' : '3m x 4m (12 sq.m)'}</p>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="bg-slate-900 text-white p-8 rounded-[2rem] mb-8 relative overflow-hidden group">
                        ...
                        <div className="relative z-10">
                          <p className="text-[10px] text-slate-500 uppercase tracking-[0.3em] font-black mb-2 leading-none">Investment Amount</p>
                          <div className="flex items-baseline justify-between">
                            <div className="flex items-baseline gap-1">
                              <span className="text-4xl font-black">{!selectedStall.includes('-') ? '₹ 9,99,999' : '₹ 3,99,999'}</span>
                            </div>
                            <span className="text-xs text-slate-500 font-bold tracking-widest">+ GST</span>
                          </div>
                        </div>
                      </div>
                      ...
                      <button
                        onClick={() => {
                          if (!selectedStall || (selectedStall && occupiedStalls.includes(selectedStall))) return;
                          const category = getPavilionInfo(selectedStall);
                          const activeEvent = events[0] || { title: "IGTF Exhibition", location: "Main Hall" };
                          ...
                          router.push(`/exhibition?${params.toString()}#registration-form`);
                        }}
                        disabled={!!(selectedStall && occupiedStalls.includes(selectedStall))}
                      ...
                      >
                        {selectedStall && occupiedStalls.includes(selectedStall) ? 'STALL SOLD OUT' : 'BOOK YOUR STALL'}
                      </button>

                      <div className="mt-8 pt-8 border-t border-slate-100 grid grid-cols-2 lg:grid-cols-3 gap-2">
                        {(!selectedStall.includes('-')
                          ? ['3 SOFAS', '3 CENTRAL TABLES', '6 CHAIRS', '15-18 LIGHTS', '1 FASCIA']
                          : ['1 SOFA', '1 CENTRAL TABLE', '2 CHAIRS', '6 LIGHTS', '1 FASCIA']
                        ).map(tag => (
                          <div key={tag} className="text-[9px] font-black text-slate-500 bg-slate-50 py-2 px-2 rounded-lg text-center tracking-wider border border-slate-100 shadow-sm uppercase">
                            {tag}
                          </div>
                        ))}
                      </div>
                    ...
                    >
                      ...
                      <h3 className="text-xl font-serif mb-3 text-slate-900">Select Your Space</h3>
                      <p className="text-sm text-slate-400 leading-relaxed max-w-[220px] mx-auto">
                        Explore the interactive layout above and choose an available exhibition booth.
                      </p>
                    </motion.div>
                  )}
                </AnimatePresence>
                ...
                <div className="bg-white p-8 rounded-2xl shadow-sm border border-slate-200/60">
                  <div className="flex items-center gap-2 mb-6">
                    ...
                    <h4 className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-400">Hall Designation</h4>
                  </div>
                  <div className="grid grid-cols-2 gap-y-6 gap-x-6">
                    {[
                      { classes: 'bg-gradient-to-br from-[#F4F5F7] to-[#D1D5DB]', label: 'Standard' },
                      { classes: 'bg-gradient-to-br from-[#FFE8CC] to-[#FDCB95]', label: 'Anchor' },
                      { classes: 'bg-gradient-to-br from-[#DEF6FF] to-[#A3E1F5]', label: 'Seating' },
                      { classes: 'bg-gradient-to-br from-[#CCD5FF] to-[#AEB9EF]', label: 'Stage Hub' },
                      { classes: 'bg-gradient-to-br from-[#FFFFFF] to-[#FCFAF0]', label: 'Main Aisle' },
                      { classes: 'bg-gradient-to-br from-[#E8F5D0] to-[#D4E7AF]', label: 'Facilities' },
                    ].map((item) => (
                      <div key={item.label} className="flex items-center gap-3">
                        <div className={`w-5 h-5 rounded-lg shadow-sm border border-black/10 shadow-[inset_0_1px_2px_rgba(255,255,255,0.7)] ${item.classes}`}></div>
                        <span className="text-xs font-black text-slate-800 tracking-tight uppercase">{item.label}</span>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="bg-white p-8 rounded-2xl shadow-sm border border-slate-200/60">
                  <div className="flex items-center gap-2 mb-6">
                    <div className="h-[2px] w-6 bg-primary rounded-full"></div>
                    <h4 className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-400">Pavilion Categories</h4>
                  </div>
                      ...
                      <table className="w-full text-left">
                        <thead className="bg-slate-50">
                          <tr>
                            ...
                            <th className="px-4 py-3 text-[10px] font-black text-slate-400 uppercase tracking-widest">Category Name</th>
                          </tr>
                        </thead>
                        <tbody className="divide-y divide-slate-50">
                          {Object.entries(pavilionCategories).sort(([a], [b]) => {
                            const numA = parseInt(a.replace('A', ''));
                            const numB = parseInt(b.replace('A', ''));
                            return numA - numB;
                          }).map(([key, category]) => {
                            const pId = key.replace('A', 'P');
                            const isSelected = selectedStall === key || selectedStall?.startsWith(`${key}-`);
                            ...
                            return (
                              <tr
                                ...
                                onClick={() => {
                                  ...
                                  const element = document.getElementById(`pavilion-${pId}`);
                                  if (element) {
                              ...
                              >
                                <td className="px-4 py-2.5">
                                  <span className={`inline-flex items-center justify-center w-7 h-7 rounded-full transition-all duration-200 text-[10px] font-black
                                ...
                                <td className={`px-4 py-2.5 text-[11px] font-bold tracking-tight transition-colors duration-200 ${isSelected ? 'text-primary' : 'text-slate-700'}`}>
                                  {category}
                                </td>
                              </tr>
                            );
